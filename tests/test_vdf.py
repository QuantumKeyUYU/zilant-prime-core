...
def test_verify_partial_proof_correct():
    ...
    assert verify_partial_proof(proofs[0], proofs[50], steps) is True
...
def test_verify_partial_proof_incorrect():
    ...
    steps = 50
    assert verify_partial_proof(seed, wrong_proof, steps) is False
...
def test_partial_proof_chain(steps, interval):
    ...
    for i in range(len(checkpoints) - 1):
        ...
        assert verify_partial_proof(
            proofs[start], proofs[end], end - start
        ) is True
