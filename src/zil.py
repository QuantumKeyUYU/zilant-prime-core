...
def pack_zil(
    ...
    return b''.join([
        ...
        struct.pack('!I', len(vdf_proof)), vdf_proof,
        struct.pack('!I', len(ciphertext)), ciphertext,

        struct.pack('!I', len(fingerprint)), fingerprint,
...
def unpack_zil(
    ...
    # salt
    salt_len = struct.unpack_from('!H', buf, off)[0]; off += 2 + salt_len

    # nonce
    nonce_len = struct.unpack_from('!H', buf, off)[0]; off += 2
    nonce = bytes(buf[off:off+nonce_len]); off += nonce_len

    # max_tries и флаг one_time
    max_tries = buf[off]; off += 1
    off += 1
    ...
    # proof
    vdf_len = struct.unpack_from('!I', buf, off)[0]; off += 4
    vdf_proof = bytes(buf[off:off+vdf_len]); off += vdf_len

    # ciphertext
    ct_len = struct.unpack_from('!I', buf, off)[0]; off += 4
    ciphertext = bytes(buf[off:off+ct_len]); off += ct_len

    # fingerprint
    fp_len = struct.unpack_from('!I', buf, off)[0]; off += 4
    stored_fp = bytes(buf[off:off+fp_len]); off += fp_len

    # metadata
    meta_len = struct.unpack_from('!I', buf, off)[0]; off += 4
    metadata = json.loads(bytes(buf[off:off+meta_len])); off += meta_len
...
def pack_zil_file(
    ...
    Path(path).write_bytes(pack_zil(
        payload, formula, lam, steps,
        key, salt, nonce,
        metadata, max_tries, one_time
    ))
...
def unpack_zil_file(
    ...
    off += 2; salt_len = struct.unpack_from('!H', buf, off-2)[0]; off += salt_len
    off += 2; nonce_len = struct.unpack_from('!H', buf, off-2)[0]
    nonce = bytes(buf[off:off+nonce_len]); off += nonce_len
    ...
    meta_off   = off
    meta_len   = struct.unpack_from('!I', buf, off)[0]; off += 4
    meta_json  = bytes(buf[off:off+meta_len]); off += meta_len
    metadata   = json.loads(meta_json)
    tries      = metadata.get('tries', 0)

    def _rewrite_meta(new_meta):
        b = json.dumps(new_meta).encode('utf-8')
        return raw[:meta_off] + struct.pack('!I', len(b)) + b + raw[meta_off+4+meta_len:]
